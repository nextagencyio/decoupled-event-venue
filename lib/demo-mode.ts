...
import venueSpacesData from '@/data/mock/spaces.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetVenueSpaces') || query.includes('nodeVenueSpaces')) {
      return loadMockData('spaces.json')
    }
    ...
    if (query.includes('GetPackages') || query.includes('nodePackages')) {
      return loadMockData('packages.json')
    }
