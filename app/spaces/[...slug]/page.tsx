...
import { GET_VENUE_SPACE_BY_PATH } from '@/lib/queries'
import { DrupalVenueSpace } from '@/lib/types'
...
interface VenueSpaceByPathData {
  route: {
    entity: DrupalVenueSpace
  } | null
}

async function getVenueSpace(path: string): Promise<DrupalVenueSpace | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<VenueSpaceByPathData>({
      query: GET_VENUE_SPACE_BY_PATH,
      variables: { path },
      fetchPolicy: 'cache-first',
    })
    return data?.route?.entity || null
  } catch (error) {
    console.error('Error fetching venue space:', error)
    return null
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/spaces/${slug.join('/')}`
  const item = await getVenueSpace(path)

  if (!item) {
    return { title: 'Venue Space Not Found | Event Venue' }
  }

  return {
    title: `${item.title} | Event Venue`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function VenueSpaceDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/spaces/${slug.join('/')}`
  const item = await getVenueSpace(path)
