...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Event Not Found | Event Venue' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).venueLocation && (
                    <div>
                      <dt className="text-sm text-gray-500">Venue Location</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).venueLocation}</dd>
                    </div>
                  )}
