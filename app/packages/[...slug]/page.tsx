...
import { GET_PACKAGE_BY_PATH } from '@/lib/queries'
import { DrupalPackage } from '@/lib/types'
...
interface PackageByPathData {
  route: {
    entity: DrupalPackage
  } | null
}
...
async function getPackage(path: string): Promise<DrupalPackage | null> {
  try {
    ...
    const { data } = await apolloClient.query<PackageByPathData>({
      query: GET_PACKAGE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/packages/${slug.join('/')}`
  const item = await getPackage(path)

  if (!item) {
    return { title: 'Package Not Found | Event Venue' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).guestCount && (
                    <div>
                      <dt className="text-sm text-gray-500">Guest Count</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).guestCount}</dd>
                    </div>
                  )}
