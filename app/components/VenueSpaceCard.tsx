...
interface VenueSpaceCardProps { item: DrupalVenueSpace }

export default function VenueSpaceCard({ item }: VenueSpaceCardProps) {
  return (
    <Link href={item.path || '#'} className="group bg-primary-900 border border-primary-800 rounded-xl hover:border-accent-500/30 hover:shadow-lg hover:shadow-accent-500/5 transition-all duration-300 overflow-hidden">
      <div className="relative h-48 bg-primary-800">
        {(item as any).image?.url ? (
          <ResponsiveImage src={(item as any).image.url} alt={(item as any).image.alt || item.title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" variations={(item as any).image.variations} targetWidth={400} />
        ) : (
      ...
      <div className="p-6">
        {(item as any).priceRange && <p className="text-sm text-accent-400 font-medium mb-2">{(item as any).priceRange}</p>}
